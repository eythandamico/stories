...
void main(){
    ...
    float a = max(clamped.r, max(clamped.g, clamped.b));
    fragColor = vec4(clamped, a);
}`
...
export default function PrismaticBurst({
  intensity = 2, speed = 0.5, animationType = 'rotate3d',
  colors, distort = 0, paused = false, offset = { x: 0, y: 0 },
  ...
  useEffect(() => {
    ...
    if ('IntersectionObserver' in window) {
      io = new IntersectionObserver(entries => { if (entries[0]) isVisibleRef.current = entries[0].isIntersecting }, { threshold: 0.01 })
    ...
    const update = now => {
      ...
      if (!isVisibleRef.current || document.hidden) { raf = requestAnimationFrame(update); return }
  ...
  useEffect(() => {
    ...
    const animMap = { rotate: 0, rotate3d: 1, hover: 2 }
    program.uniforms.uAnimType.value = animMap[animationType ?? 'rotate']
    ...
    if (Array.isArray(colors) && colors.length > 0) {
      ...
      gradTex.minFilter = gl.LINEAR; gradTex.magFilter = gl.LINEAR
      gradTex.wrapS = gl.CLAMP_TO_EDGE; gradTex.wrapT = gl.CLAMP_TO_EDGE
